...
// Take photo  trigger
#define TAKE_PHOTO "/take_photo"

// True if capture is already in progress
boolean workInProgress = false;

// True if command to take picture is received
...
void initCamera()
{
  ...
  if (psramFound())
  {
    ...
    config.jpeg_quality = 10; // better performances with lower quality
    config.fb_count = 1;
  }
  else
  {
...
void setup()
{
  ...
  server.on("/take_photo", HTTP_GET, [](AsyncWebServerRequest *request)
          { 
            Serial.println("requesting takePhoto");
            takePhoto = true; 
          });
